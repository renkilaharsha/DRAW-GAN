...
def save_reconimage(x,count=0):
    for t in range(T):
        img = xrecons_grid(x[t],B,A)
        plt.matshow(img, cmap=plt.cm.gray)
        imgname = 'recon_image/count_%d_%s_%d.png' % (count,'test', t)  # you can merge using imagemagick, i.e. convert -delay 10 -loop 0 *.png mnist.gif
        plt.savefig(imgname)
        print(imgname)
def save_inputimage(x,count=0):
    img = xrecons_grid(x,B,A)
    plt.matshow(img, cmap=plt.cm.gray)
    imgname = 'input_image/count_%d_%s.png' % (count,'test')  # you can merge using imagemagick, i.e. convert -delay 10 -loop 0 *.png mnist.gif
    plt.savefig(imgname)
    print(imgname)
