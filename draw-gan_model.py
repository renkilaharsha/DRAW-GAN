...
class DrawModel(nn.Module):
    ...
    '''def inp_out_image(self,x,count=0):
        for t in range(T):
            img = x.cpu().numpy().reshape(self.batch_size, 28, 28)
            img = xrecons_grid(img, B, A)
   
            plt.matshow(img, cmap=plt.cm.gray)
            imgname = 'inpimage/inp_%d_%s_%d.png' % (count,'test', t)  # you can merge using imagemagick, i.e. convert -delay 10 -loop 0 *.png mnist.gif
            plt.savefig(imgname)
        print(imgname)'''
